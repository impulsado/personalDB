...
class CheckListItemsDatabaseHelper implements DatabaseHelperCommon {
  @override
  Future<int> createItem(Map<String, dynamic> data, String password) async {
    final db = await DatabaseHelper.db(password);
    final id = await db.insert("CheckListItems", data, conflictAlgorithm: sql.ConflictAlgorithm.replace);
    return id;
  }
  ...
  Future<int> updateItem(int id, Map<String, dynamic> data, String password) async {
    final db = await DatabaseHelper.db(password);
    final result = await db.update("CheckListItems", data, where: "id = ?", whereArgs: [id]);
    return result;
  }
  ...
  Future<List<Map<String, dynamic>>> getItems(String password) async {
    final db = await DatabaseHelper.db(password);
    return db.query("CheckListItems", orderBy: "id");
  }

  @override
  Future<List<Map<String, dynamic>>> getItem(int id, String password) async {
    final db = await DatabaseHelper.db(password);
    return db.query("CheckListItems", where: "id = ?", whereArgs: [id], limit: 1);
  }
  ...
  Future<void> deleteItem(int id, String password) async {
    final db = await DatabaseHelper.db(password);
    try {
      await db.delete("CheckListItems", where: "id = ?", whereArgs: [id]);
    } catch (err) {
      debugPrint("Something went wrong when deleting an item: $err");
    }
  }

  Future<List<Map<String, dynamic>>> getItemsByChecklistId(int checklistId, String password) async {
    final db = await DatabaseHelper.db(password);
    return db.query("CheckListItems", where: "checklist_id = ?", whereArgs: [checklistId], orderBy: "id");
  }
}