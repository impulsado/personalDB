...
class PasswordsDatabaseHelper implements DatabaseHelperCommon {
  @override
  Future<int> createItem(Map<String, dynamic> data, String password) async {
    final db = await DatabaseHelper.db(password);
    final id = await db.insert("passwords", data, conflictAlgorithm: sql.ConflictAlgorithm.replace);
    return id;
  }
  ...
  Future<int> updateItem(int id, Map<String, dynamic> data, String password) async {
    final db = await DatabaseHelper.db(password);
    final result = await db.update("passwords", data, where: "id = ?", whereArgs: [id]);
    return result;
  }
