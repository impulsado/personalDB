...
import 'package:personaldb/widgets/search/search_entertainment.dart';
import 'package:personaldb/main.dart';
import 'package:personaldb/database/database_helper_entertainment.dart';

void main() {
  runApp(const MyAppEntertainment());
}

class MyAppEntertainment extends StatelessWidget {
  const MyAppEntertainment({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      theme: Themes.light,
      home: CategoryEntertainment(MyCategory()),
    );
  }
}
...
class _CategoryEntertainmentState extends State<CategoryEntertainment> with TickerProviderStateMixin {
      ...
      child: Column(
        children: [
          Padding(
            ...
            child: SearchBarEntertainment(
              searchController: _searchController,
              focusNode: _focusNode,
              enableOrdering: true,
              loadItemsFunction: () async {
                return await EntertainmentDatabaseHelper().getAuthor(MyApp.dbPassword!);
              },
                ...
                setState(() {
                  ...
                  switch (result) {
                    case "Title":
                      ...
                      break;
                    case "Author":
                      _notes.sort((a, b) => _isAscending ? a["author"].toString().compareTo(b["author"].toString()) : b["author"].toString().compareTo(a["author"].toString()));
                      break;
                    case "Rate":
